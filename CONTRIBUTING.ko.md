...
사용자는 GitHub의 스킬 폴더에서 `$skill-installer`로 직접 설치합니다. 아래 `make` 대상은 로컬 기여 및 검증 하네스에서만 사용합니다.
...
- frontmatter의 `name`과 `description` 값에 JSON 형식의 따옴표를 사용합니다.
...
4. 생성기가 패밀리 및 최상위 카탈로그를 갱신했는지 확인합니다.
